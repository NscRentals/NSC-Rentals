...
export async function addSpareParts(req, res) {
    ...
    try{
        // Generate a unique partId with SP prefix and 6 random digits
        const randomNum = Math.floor(Math.random() * 1000000).toString().padStart(6, '0');
        const partId = `SP${randomNum}`;
        
        // Add the generated partId to the data
        const data = { ...req.body, partId };
        const newSpareParts = new sparePartsInventory(data);
        
        await newSpareParts.save();
        res.json({
            message : "Spare parts added successfully",
            sparePart: newSpareParts
        });
     }catch(error){
        res.status(500).json({
